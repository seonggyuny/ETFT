...
def compute_metrics(
    ...
    if ptu.dist_rank == 0:
        ...
        for i in range(len(ret_metrics["IoU"])):
            print(100 * ret_metrics["IoU"][i])
...
def compute_metrics_sg(
    ...
    if ptu.dist_rank == 0:
        ...
        list_seg_pred.append(np.asarray(seg_pred))
        list_seg_gt.append(np.asarray(seg_gt))
        ret_metrics = mean_iou(
            results=list_seg_pred,
            gt_seg_maps=list_seg_gt,
            num_classes=n_cls,
